...
在输入目录同级新建 `input-bak` 和 `input-out`。独立 CLASS 先按字节码类名检查白名单和排除规则，
未通过的文件直接跳过，不创建备份文件或对应目录。对通过筛选的 CLASS 和候选 JAR，先复制原始文件到备份位置，
...
不保留对应备份或输出；删除临时备份后会清理空子目录，但保留 `input-bak` 根目录和其他有效备份。
普通资源不复制。JAR 内至少一个 CLASS 修改才覆盖并输出**完整 JAR**，
